...
fixed_effect <- fixef(model_painimprove_rs)
fixed_intercept <- fixed_effect["(Intercept)"]
fixed_slope <- fixed_effect["AvePain_c"] ## Fixed effects

random_effects <- ranef(model_painimprove_rs)$ID
random_effects_df <- data.frame(
  ID = rownames(random_effects),
  RandomIntercept = random_effects[, "(Intercept)"],
  RandomSlope = random_effects[, "AvePain_c"]
) ## Random effects
...
fixed_effect <- fixef(model_sleepimprove_rs)
fixed_intercept <- fixed_effect["(Intercept)"]
fixed_slope <- fixed_effect["Sleep_c"] ## Fixed effects

random_effects <- ranef(model_sleepimprove_rs)$ID
random_effects_df <- data.frame(
  ID = rownames(random_effects),
  RandomIntercept = random_effects[, "(Intercept)"],
  RandomSlope = random_effects[, "Sleep_c"]
) ## Random effects
...
ggplot() +
  ...
  theme_minimal() +
  labs(
    title = "Random Slope Plot: Sleep vs. Global Improvement",
    x = "Sleep (Centered)",
    y = "Global Improvement"
  ) +
  theme(
...
fixed_effect <- fixef(model_moodimprove_rs)
fixed_intercept <- fixed_effect["(Intercept)"]
fixed_slope <- fixed_effect["Mood_c"] ## Fixed effects

random_effects <- ranef(model_moodimprove_rs)$ID
random_effects_df <- data.frame(
  ID = rownames(random_effects),
  RandomIntercept = random_effects[, "(Intercept)"],
  RandomSlope = random_effects[, "Mood_c"]
) ## Random effects
...
# Extract percentiles of the moderator (PCS)
percentiles <- quantile(df_new$B_Psych_PCS, probs = c(0, 0.25, 0.5, 0.75, 1))

# Create new data for prediction with different moderator values
new_data <- expand.grid(AvePain_c = seq(min(df_new$AvePain_c, na.rm = TRUE), max(df_new$AvePain_c, na.rm = TRUE), length.out = 100),
                        ...
                        ID = unique(df_new$ID)[1]) # Use the first ID as a reference
...
# Plot the graph
ggplot(df_new, aes(x = AvePain_c, y = GlobalImprovement, color = B_Psych_PCS)) +
  geom_line(data = new_data, aes(group = B_Psych_PCS), size = 1) +
  scale_color_gradientn(colors = c("darkorchid", "darkblue", "steelblue", "mediumseagreen", "yellow"),
                        name = "Moderator (PCS)",
...
```{r}
# Lev1_PainInterference (IV) and Lev1_improvement (outcome) 
# as a function of Lev2_PCS (moderator) 
```
...
library(viridis)
set.seed(123)
data <- expand.grid(distance = seq(0, 42, by = 1), resilience_percentile = seq(5, 95, by = 10))
data <- data %>% 
  mutate(oxygen_cost = resilience_percentile * (distance / 42) + rnorm(n(), 0, 2))
ggplot(data, aes(x = distance, y = oxygen_cost, group = resilience_percentile, color = resilience_percentile)) +
  geom_line(size = 0.7, alpha = 0.6) +
  geom_line(data = data %>% filter(resilience_percentile == 50), aes(x = distance, y = oxygen_cost), color = "white", size = 1.5) +
  scale_color_viridis(name = "Resilience percentile", option = "plasma", direction = -1) +
  labs(
    title = "Most runners get less efficient late in the marathon",
    subtitle = "Based on simulated data",
    x = "Distance (km)",
    y = "Change in oxygen cost (%)"
  ) +
  theme_minimal(base_size = 15) +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
